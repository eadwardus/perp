...
#define CHAR_CMPI(c1,c2) (tolower((int)(c1)) == tolower((int)(c2)))

/* cstr_matchi()
**   synopsis:
**     match string against pattern
**     pattern may include:
**       - any literal character, matched case-insensitive
...
int
cstr_matchi(const char *pattern, const char *string)
{
  ...
  while((*string != '\0') && (*pattern != '*')){
      if(!(CHAR_CMPI(*pattern, *string) || (*pattern == '?'))){
  ...
  */
  while(*string != '\0'){
      if(*pattern == '*'){
          ...
          /* mark next substring: */
          s = &string[1];
      }else if(CHAR_CMPI(*pattern, *string) || (*pattern == '?')){
